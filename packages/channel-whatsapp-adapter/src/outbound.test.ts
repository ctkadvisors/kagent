...
describe('deliverOutboundTurns', () => {
  it('sends a completed task result once and records the delivered task ref', async () => {
    ...
    expect(socket.sendMessage).toHaveBeenCalledWith('15551234567@s.whatsapp.net', {
  ...
  it('backs off the session when WhatsApp send fails', async () => {
    const store = makeStore({
      sessions: [makeSession()],
      tasks: [
        makeTask({ status: { phase: 'Completed', result: { content: 'Daily check done' } } }),
    ...
    const socket = makeSocket();
    socket.sendMessage.mockRejectedValueOnce(new Error('network unavailable'));
  ...
  it('marks the session failed after the configured outbound failure limit', async () => {
    ...
    await deliverOutboundTurns({ config, store, socket, logger: quietLogger, clock });
...
function makeSocket(): WhatsAppSocketLike & {
