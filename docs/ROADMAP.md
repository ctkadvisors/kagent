...
- [x] **Move Agent + Task spec injection off env JSON.** `KAGENT_AGENT_SPEC` and `KAGENT_TASK_SPEC` as env-var JSON strings is fine for smoke tests but caps payloads at ARG_MAX and exposes secrets to `ps`/`/proc`. Switch to a downward-API volume or a per-task ConfigMap mounted at a known path before the first real consumer. **Landed 2026-04 via v0.2.0-typed-io:** operator now mounts `/var/kagent/config/{agent,task}.spec.json` from a per-task ConfigMap; `parseEnv` prefers the file path and falls back to env-JSON for one release. `KAGENT_SPEC_SOURCE` (= `'configmap' | 'env-json' | 'mixed'`) is stamped on `process.env` and surfaced in OTel attribute `kagent.spec.source` for on-call grep. **Audit-rev2 M11:** the env-JSON fallback is targeted for removal in **v0.3.0-cas** (next major); a ConfigMap-mount failure should produce a `Failed` AgentTask with a structured error rather than silently rolling over to the deprecated path. Until then, the single-release back-compat tail stays.
...
- [x] Wire the operator's reconcile to USE the task-graph helpers (P4 wire-up): on child status change, look up parent and patch parent.status.children + aggregatePhase. Audit: `parent.children_aggregated` per parent-update (before/after counter snapshots). Supervision-aware ordering: when a child reaches `Failed` and supervision is enabled, supervision runs first so the parent projection reflects post-supervision state. Idempotent — projection-unchanged paths skip the patch AND the audit emission.
