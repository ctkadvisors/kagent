...
### 4.4 Recommendation — **Option A (bridge)**

The substrate-shape principle in `SUBSTRATE-V1.md` §5 is *"Primitives are orthogonal."* A2A is HTTP-shaped; kagent's Event primitive is NATS-shaped. Forcing A2A onto NATS subjects breaks orthogonality without solving the problem A2A actually solves (which is HTTP/JSON-RPC interop with 150+ HTTP-shaped peers). The bridge is the right composition.
