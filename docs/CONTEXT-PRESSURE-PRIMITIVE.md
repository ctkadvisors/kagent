...
Five-row × five-column scorecard, restated from R2 §4 of the May 2026 audit (`evidence/audit-rev2/R2.md`) and verified by repository / docs read on 2026-05-06.

| Capability | kagent v0.1.9 | Anthropic Claude Code | Cloudflare Project Think | Google Vertex Memory Bank | OpenAI Agents SDK 0.14.0 |
|---|---|---|---|---|---|
...
| **Substrate refuses the next LLM call at a fixed % threshold (default 0.95)** | ✅ `LLMClientHttpError(0, 'context_window_substrate_refused')` ([executor.ts:509-544](../packages/agent-loop/src/executor.ts)) | ❌ relies on auto-compaction; circuit-breaker is "3 failed compactions", not a % threshold ([Claude API compaction docs](https://platform.claude.com/docs/en/build-with-claude/compaction)) | ❌ relies on macro/micro compaction in the harness | ❌ relies on Memory Bank automated triggers | ❌ relies on upstream provider's 400 |
| **Substrate auto-compacts (this row is inverted — ❌ here = "we don't")** | ❌ explicit non-goal ([CONTEXT-AWARENESS.md §2](./CONTEXT-AWARENESS.md)) | ✅ at ~95% ([issue 25679](https://github.com/anthropics/claude-code/issues/25679)) | ✅ macro + micro, non-destructive, SQLite-backed | ✅ continuous event streaming | ⚠ via SDK memory mgmt; configurable, defaults on |
...
**Reading.** No single competitor combines (a) substrate-side refusal, (b) in-agent introspection, (c) an "agent ignored its own budget" detector, and (d) operator-visible audit emission for the pressure decision. The hosted vendors are racing to make context magic invisible — that is the UX bet, not an oversight. kagent is intentionally betting the other way for the long-running / regulated workloads where invisibility is the bug.
