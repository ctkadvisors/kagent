...
- **No agent SDK.** Agents inside pods run any framework (Strands TS, Mastra, the forked AgentExecutor); the substrate is framework-agnostic. Agents that don't use `@kagent/agent-loop` lose the v0.1.9+ context-safety guarantees (the 95% pre-call refusal + the `context_pressure_ignored` detector — see [`CONTEXT-AWARENESS.md`](./CONTEXT-AWARENESS.md) and the audit-rev2 R3 re-run at `evidence/audit-rev2/R3.md`) **unless** they install a per-framework adapter that bridges the framework's hooks to the substrate's contracts. As of v0.1.9 a reference adapter ships at [`packages/agent-loop-vercel-ai`](../packages/agent-loop-vercel-ai/) — wraps Vercel AI SDK v6's `wrapLanguageModel` middleware + `streamText` lifecycle so the four substrate guarantees (context-window safety-net, capability JWT, substrate tools, context-pressure detector) survive. The substrate does NOT enforce the safety-net at the network boundary — third-party in-pod runtimes that ignore `KAGENT_AGENT_MODEL_CONTEXT_WINDOW` will run to upstream `400 context_length_exceeded` regardless of the chart values. Pick `@kagent/agent-loop` (the reference loop) for the simplest path; reach for the Vercel AI SDK adapter when the agent's design already commits to AI-SDK-shaped tools or middleware.
