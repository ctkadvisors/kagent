...
1. **`agentName ∈ Agent.spec.allowedChildAgents`** of the parent agent — OR (per `v0.1.3-prefix-allow`) the target Agent's `kagent.knuteson.io/from-template` label is in `Agent.spec.allowedChildTemplates`. Both lists union; an Agent without the from-template label is never admitted via the templates list (fail-closed). When BOTH lists are empty/unset → `policy_denied: this agent has no allowedChildAgents`. The agent-pod reads its own Agent spec at boot and (when allowedChildTemplates is configured and the name isn't in the cheap exact-match path) does one `GET /agents/{name}` to read the target's labels — its existing `agents: [get]` RBAC suffices.
