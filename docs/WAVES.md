...
3. **SHIPPED.** Capability-issuer (`packages/operator/src/cap-issuer.ts`) + reconciler wiring: every admitted AgentTask can mint a per-task Secret-backed cap JWT when `KAGENT_CAPABILITIES_ENABLED=true`. The operator resolves Agent claims, narrows by parent bundle, stamps Tenant claims/issuer overrides, applies the KeyRotation TTL policy when enabled, patches `status.capabilityRef`, mounts the JWT into the Job, and emits `capability.minted` plus `keyrotation.cap_minted_with_ttl` audit events.
...
7. **SHIPPED (v0.1.7-rig.2).** `verify_completion` substrate hook: schema field shipped in v0.3.0-capabilities (`AgentTask.spec.verifyContract: { scriptRef?, llmJudgePromptRef? }` + `status.verification`); the reconciler-side runner now lives in `packages/operator/src/verifier.ts`. Two dispatch paths: `scriptRef` spawns a one-shot K8s Job (ownerRef → AgentTask, ConfigMap-mounted input.json carrying parent's `result` + `payload`, `activeDeadlineSeconds=60`); `llmJudgePromptRef` fetches the prompt from Langfuse, renders `{{outputs}}` with parent.status.result JSON, POSTs to `KAGENT_LLM_GATEWAY_BASE_URL/chat/completions`, parses `{verdict, reason}`. Misconfig (both refs / neither ref / gateway-unconfigured / langfuse-unconfigured) fail-closes with structured `verifier_misconfig:*` reason. Idempotent on re-fire (re-read inside the per-uid mutex). Three new audit events (`verifier.started/completed/failed`) bring the catalog 44 → 47. Default OFF (`verifier.enabled=false`) — same posture as `agentPod.spawnChild`. 41 tests in `verifier.test.ts`.

**Validation:** ✅ The schema gate, narrowing predicate, JWT sign+verify round-trip, reconciler Secret mount, Tenant issuer stamping, TTL audit emission, agent-pod verification, and spawn narrowing paths are covered in unit-level tests. The remaining validation surface is live-cluster evidence: prove the ArgoCD-deployed chart has the signing Secret, JWKS service, audit stream, and agent-pod cap mount healthy together.
...
**Status:** SHIPPED — all 6 deliverables landed; no deferrals. New `@kagent/keyrotation-controller` package: 37 tests across 4 files (svid-policy, cap-ttl, gateway-rotation, chaos). Operator: 971 → 982 tests (+11), plus GA hardening wiring now threads the resolved cap TTL policy into per-task minting and emits `keyrotation.cap_minted_with_ttl`. Audit-events: 42 → 46 tests (+4). Helm wiring: `keyRotation:` block under Wave 4 marker; `KAGENT_KEYROTATION_*` envs additive (default-OFF gradual roll-out); no new RBAC verbs (rotation reads existing Secrets + uses the existing audit pipeline). Chaos test design: pure-function + virtual-clock simulation rather than real K8s/SPIRE/gateway — sufficient for the substrate's contract (rotation triple MUST NOT cause an in-flight gateway call to fail) and runs in milliseconds; integration with real SPIRE deferred to a future Wave 4.x release.
...
| `verify_completion` hook | W2 / Caps (schema) + Phase 5 / Verifier (runner) | v0.3.0 schema; v0.1.7-rig.2 runner | ✓ |
