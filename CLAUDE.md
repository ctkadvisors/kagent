...
**Status:** shipped and running as the homelab fleet's substrate and deployed via ArgoCD from `../new_localai/k8s-kustomized/overlays/production/kagent/`, where every component is pinned to one tag. The numeric project stats that used to live in this banner (package count, source lines, test file count, pinned tag) are intentionally omitted here: they go stale with every merge, so treat them as informational, not authoritative, and verify current state yourself with `pnpm -r test` and `git describe` rather than trusting the numbers below. Read `.planning/STATE.md` §"Reconciliation note (2026-09-02)" for what shipped in the 2026-08-30..09-02 burst and what is still open (operator `main.ts` size, shared http/k8s packages, code behind off-flags).
