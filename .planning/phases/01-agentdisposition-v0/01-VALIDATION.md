...
| 01-03-T1 | 03   | 2    | DISP-03     | DispositionOverlayRow shared DTO in @kagent/dto; postsToday: 0 literal type; runtime guard exists                                                               | unit (type + runtime guard)     | `pnpm --filter @kagent/dto test --run -- disposition`                                                                                                                                                                   | ✅ W0         | ⬜ pending                     |
| 01-03-T2 | 03   | 2    | DISP-03     | /api/dispositions route computes the projection from gateway DTOs + ConfigMap annotations; over_budget exactly-once-per-(agentRef,reason)-per-UTC-day; 15 tests | unit (mocked coreApi + gateway) | `pnpm --filter @kagent/workbench-api test --run -- dispositions`                                                                                                                                                        | ✅ W0         | ⬜ pending                     |
| 01-03-T3 | 03   | 2    | DISP-03     | Route mounted in router.ts; AuditPublisher wired conditionally; Helm value+env var plumbed end-to-end in `packages/operator/charts/kagent-workbench/`           | full package + helm lint        | `pnpm --filter @kagent/workbench-api test --run && helm lint packages/operator/charts/kagent-workbench`                                                                                                                 | ✅ W0         | ⬜ pending                     |
| 01-04-T1 | 04   | 3    | DISP-04     | fetchDispositions + state hook wiring; api.test.ts + state.test.ts (6 tests covering fetch, validate, Map keying, SSE refetch, mount-time refetch)              | unit                            | `pnpm --filter @kagent/workbench-ui test --run -- api.test state.test`                                                                                                                                                  | ✅ W0         | ⬜ pending                     |
| 01-04-T2 | 04   | 3    | DISP-04     | source-binding helper enforces D7 Prime Directive in dev; no-op in prod; 6 tests                                                                                | unit                            | `pnpm --filter @kagent/workbench-ui test --run -- source-binding`                                                                                                                                                       | ✅ W0         | ⬜ pending                     |
| 01-04-T3 | 04   | 3    | DISP-04     | DispositionOverlay component + reload-stability snapshot test + base-building-only mode + source-field assertion fires; 10 tests                                | unit + snapshot                 | `pnpm --filter @kagent/workbench-ui test --run -- DispositionOverlay`                                                                                                                                                   | ✅ W0         | ⬜ pending                     |
| 01-04-T4 | 04   | 3    | DISP-04     | DispositionOverlay mounted in CommandView; production build green                                                                                               | typecheck + build               | `pnpm --filter @kagent/workbench-ui typecheck && pnpm --filter @kagent/workbench-ui build`                                                                                                                              | ✅ W0         | ⬜ pending                     |
| 01-04-T5 | 04   | 3    | DISP-04     | Manual: end-to-end overlay rendering against a running workbench; reload-stability + data-source-field attributes + VITE_PRESSURE_DRAMATIZATION toggle          | manual                          | n/a (visual checkpoint per task <how-to-verify> steps)                                                                                                                                                                  | n/a (browser) | ⬜ pending (manual checkpoint) |
